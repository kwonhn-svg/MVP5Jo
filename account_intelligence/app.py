...
from main import run_agent, save_report

load_dotenv()

# ── 섹션 아이콘 ───────────────────────────────────────────────────────────────
SECTION_ICONS = {
    "기업 기본 정보": "🏢", "최근 동향": "📰", "채용 신호": "👥",
    "재무·공시 동향": "📊", "기술 스택": "⚙️", "경쟁사 현황": "⚔️",
    "의사결정자 프로파일": "👤", "구매 신호": "⏰", "영업 인사이트": "💡",
    "미팅 시나리오": "🗓️", "준비 체크리스트": "✅",
}
FULL_WIDTH = {"기업 기본 정보", "영업 인사이트", "미팅 시나리오", "준비 체크리스트"}
...
def inline(text: str) -> str:
    ...
    text = re.sub(
        ...
        r'style="color:#4f6ef7;text-decoration:underline;word-break:break-all;font-size:12.5px;">\1</a>',
        text
    )
    return text
...
"""
    ...
    <div style="{col_span}background:#fff;border-radius:12px;overflow:hidden;
      ...
      <details open>
        <summary style="background:#f8fafc;border-left:4px solid #4f6ef7;padding:12px 18px;
                        display:flex;align-items:center;gap:8px;cursor:pointer;list-style:none;
                        user-select:none;">
          <span style="font-size:18px;">{icon}</span>
          <span style="font-size:14px;font-weight:700;color:#374151;flex:1;">{sec_title}</span>
          <span class="toggle-arrow" style="font-size:12px;color:#9ca3af;transition:transform 0.2s;">▲</span>
        </summary>
...
"""
    if sections:
      ...
      <div style="{col_span}background:#fff;border-radius:12px;overflow:hidden;
                   box-shadow:0 1px 6px rgba(0,0,0,0.07);border:1px solid #e5e7eb;">
        <details open>
          <summary style="background:#f0f4ff;border-left:4px solid #4f6ef7;padding:12px 18px;
                          display:flex;align-items:center;gap:8px;cursor:pointer;list-style:none;
                          user-select:none;">
            <span style="font-size:18px;">{icon}</span>
            <span style="font-size:14px;font-weight:700;color:#374151;flex:1;">{sec_title}</span>
...
details[open] summary .toggle-arrow { transform:rotate(0deg); }
details:not([open]) summary .toggle-arrow { transform:rotate(180deg); }
#submit-btn {
